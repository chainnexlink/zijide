...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { origin, destination, mode = 'walking', avoid = [] } = body;
    ...
    const { data: dangerRows } = await admin.from('alerts').select('id,title,severity,latitude,longitude,affected_radius_km').is('end_time', null).not('latitude', 'is', null).not('longitude', 'is', null).limit(100);
    ...
    const routes = (directions.routes || []).map((route: any, index: number) => {
      ...
      return { id: `route-${index}`, summary: route.summary || `?? ${index + 1}`, coordinates, distanceMeters: leg.distance?.value || 0, distanceText: leg.distance?.text || '', durationSeconds: leg.duration?.value || 0, durationText: leg.duration?.text || '', safetyScore: Math.max(0, 100 - dangerHits.reduce((sum: number, item: any) => sum + (item.severity === 'red' ? 35 : item.severity === 'orange' ? 20 : 10), 0)), dangerWarnings: dangerHits.map((item: any) => ({ id: item.id, title: item.title, severity: item.severity })) };
    });
...
function validPoint(value: any) { return value && Number.isFinite(Number(value.latitude)) && Number.isFinite(Number(value.longitude)); }
