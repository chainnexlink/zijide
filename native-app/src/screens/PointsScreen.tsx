...
export function PointsScreen({ navigation }: Props) {
  ...
  const redeem = (packageId: 'd7' | 'd30', planId: 'personal' | 'family', points: number, days: number) => Alert.alert('确认兑换', `使用 ${points} 积分兑换 ${planId === 'family' ? '家庭版' : '个人版'} ${days} 天？`, [{ text: '取消', style: 'cancel' }, { text: '兑换', onPress: async () => { setBusy(true); const { data, error } = await supabase.functions.invoke('redeem-points', { body: { packageId, planId } }); setBusy(false); if (error || !data?.success) Alert.alert('兑换失败', data?.error === 'INSUFFICIENT_POINTS' ? `积分不足，需要 ${data.required} 分` : data?.error || error?.message); else { Alert.alert('兑换成功', `订阅已延长 ${data.daysAdded} 天，到期时间 ${new Date(data.expiresAt).toLocaleString('zh-CN')}`); await load(); } } }]);
  if (loading) return <View style={styles.loading}><ActivityIndicator color={colors.warning} size="large" /></View>;
  return <Screen title="互助积分" subtitle="救援贡献与兑换记录" action={<Pressable onPress={() => navigation.goBack()}><Text style={styles.back}>返回</Text></Pressable>} refreshing={busy} onRefresh={() => void load()}>
    <View style={styles.wallet}><Text style={styles.balance}>{wallet.balance}</Text><Text style={styles.balanceLabel}>可用积分</Text><View style={styles.stats}><Stat value={wallet.total_earned} label="累计获得" /><Stat value={wallet.total_spent} label="累计使用" /></View></View>
    <View style={styles.card}><Text style={styles.title}>兑换免费订阅时长</Text><Text style={styles.note}>积分不会抵扣 App Store 内购金额，而是直接兑换免费订阅天数，符合 Apple 支付规则。</Text><Package title="7天体验" points={500} disabled={wallet.balance < 500 || busy} onPersonal={() => redeem('d7', 'personal', 500, 7)} onFamily={() => redeem('d7', 'family', 500, 7)} /><Package title="30天订阅" points={2000} disabled={wallet.balance < 2000 || busy} onPersonal={() => redeem('d30', 'personal', 2000, 30)} onFamily={() => redeem('d30', 'family', 2000, 30)} /></View>
    <View style={styles.card}><Text style={styles.title}>积分明细</Text>{rows.length ? rows.map((row) => <View key={row.id} style={styles.transaction}><View style={styles.transactionBody}><Text style={styles.reason}>{row.reason || labelType(row.type)}</Text><Text style={styles.date}>{new Date(row.created_at).toLocaleString('zh-CN')}</Text></View><Text style={[styles.amount, { color: row.amount >= 0 ? colors.safe : colors.danger }]}>{row.amount >= 0 ? '+' : ''}{row.amount}</Text></View>) : <Text style={styles.empty}>暂无积分记录，参与互助救援可获得积分。</Text>}</View>
  </Screen>;
}

function Package({ title, points, disabled, onPersonal, onFamily }: { title: string; points: number; disabled: boolean; onPersonal: () => void; onFamily: () => void }) { return <View style={styles.package}><View><Text style={styles.packageTitle}>{title}</Text><Text style={styles.packageMeta}>{points} 积分</Text></View><View style={styles.packageActions}><Pressable disabled={disabled} style={[styles.redeem, disabled && styles.disabled]} onPress={onPersonal}><Text style={styles.redeemText}>个人版</Text></Pressable><Pressable disabled={disabled} style={[styles.redeem, disabled && styles.disabled]} onPress={onFamily}><Text style={styles.redeemText}>家庭版</Text></Pressable></View></View>; }
...
function labelType(type: string) { return type.startsWith('earn') ? '获得积分' : type.startsWith('spend') ? '使用积分' : '积分变动'; }
