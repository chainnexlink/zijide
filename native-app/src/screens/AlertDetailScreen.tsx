...
export function AlertDetailScreen({ route, navigation }: Props) {
  ...
  return <Screen title="预警详情" subtitle={alert.is_verified ? '已核验信息' : '待进一步核验'} action={<Pressable onPress={() => navigation.goBack()}><Text style={styles.back}>返回</Text></Pressable>}>
    ...
    <View style={styles.map}><View style={[styles.range, { borderColor: tone }]}><View style={[styles.marker, { backgroundColor: tone }]} /></View><Text style={styles.mapText}>{alert.latitude != null && alert.longitude != null ? `${Number(alert.latitude).toFixed(4)}, ${Number(alert.longitude).toFixed(4)}` : '预警区域坐标待确认'}</Text>{alert.latitude != null && alert.longitude != null ? <Pressable onPress={() => void Linking.openURL(`https://maps.apple.com/?ll=${alert.latitude},${alert.longitude}`)}><Text style={styles.link}>在地图中查看影响区域</Text></Pressable> : null}</View>
...
const styles = StyleSheet.create({ loading: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background }, back: { color: colors.info, fontWeight: '800' }, banner: { backgroundColor: colors.surface, borderWidth: 1, borderRadius: radius.lg, padding: spacing.lg, gap: spacing.sm }, level: { fontWeight: '900', fontSize: 12 }, alertTitle: { color: colors.text, fontSize: 24, fontWeight: '900' }, location: { color: colors.info }, card: { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: radius.md, padding: spacing.lg, gap: spacing.md }, section: { color: colors.text, fontSize: 18, fontWeight: '900' }, row: { flexDirection: 'row', justifyContent: 'space-between', gap: spacing.md }, label: { color: colors.muted }, value: { color: colors.text, fontWeight: '700', flex: 1, textAlign: 'right' }, description: { color: '#CBD5E1', lineHeight: 22 }, link: { color: colors.info, fontWeight: '800' }, map: { height: 220, borderRadius: radius.md, backgroundColor: '#101D2B', borderColor: colors.border, borderWidth: 1, alignItems: 'center', justifyContent: 'center', gap: spacing.md, overflow: 'hidden' }, range: { width: 130, height: 130, borderRadius: 65, borderWidth: 2, borderStyle: 'dashed', backgroundColor: '#FFFFFF08', alignItems: 'center', justifyContent: 'center' }, marker: { width: 22, height: 22, borderRadius: 11 }, mapText: { color: colors.muted, fontSize: 12 }, step: { flexDirection: 'row', alignItems: 'center', gap: spacing.md }, stepNo: { width: 28, height: 28, borderRadius: 14, textAlign: 'center', lineHeight: 28, color: colors.white, fontWeight: '900' }, stepText: { color: '#E2E8F0', flex: 1, lineHeight: 20 }, shelter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: spacing.sm, borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth }, shelterName: { color: colors.text, fontWeight: '800' }, shelterAddress: { color: colors.muted, fontSize: 12, marginTop: 4 }, chevron: { color: colors.muted, fontSize: 26 } });
