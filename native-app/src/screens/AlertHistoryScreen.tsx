...
export function AlertHistoryScreen({ navigation }: Props) {
  ...
  return <Screen title="????" subtitle={`${offline ? '???? ? ' : ''}??? ${rows.length} ?`} action={<Pressable onPress={() => navigation.goBack()}><Text style={styles.back}>??</Text></Pressable>}>
    <View style={styles.filters}><Text style={styles.label}>????</Text><View style={styles.options}>{(['all', 'red', 'orange', 'yellow'] as const).map((value) => <Filter key={value} label={{ all: '??', red: '??', orange: '??', yellow: '??' }[value]} active={severity === value} onPress={() => setSeverity(value)} />)}</View><Text style={styles.label}>????</Text><View style={styles.options}>{[1, 7, 30, 90].map((value) => <Filter key={value} label={value === 1 ? '24??' : `${value}?`} active={days === value} onPress={() => setDays(value)} />)}</View></View>
    {rows.map((alert) => <Pressable key={alert.id} style={styles.card} onPress={() => navigation.navigate('AlertDetail', { alertId: alert.id })}><View style={styles.top}><Text style={[styles.level, { color: alert.severity === 'red' ? colors.danger : alert.severity === 'orange' ? colors.warning : '#EAB308' }]}>{alert.severity.toUpperCase()}</Text><Text style={styles.time}>{alert.created_at ? new Date(alert.created_at).toLocaleString('zh-CN') : ''}</Text></View><Text style={styles.title}>{alert.title}</Text><Text style={styles.meta}>{alert.alert_type} ? {[alert.city, alert.country].filter(Boolean).join(' ') || '????'}</Text></Pressable>)}
    {!rows.length && !loading ? <Text style={styles.empty}>??????????</Text> : null}
    {loading ? <ActivityIndicator color={colors.info} /> : hasMore ? <Pressable style={styles.more} onPress={() => void load(page + 1)}><Text style={styles.moreText}>????</Text></Pressable> : rows.length ? <Text style={styles.end}>???????</Text> : null}
  </Screen>;
}
